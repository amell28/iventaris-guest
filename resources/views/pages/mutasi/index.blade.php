@extends('layouts.guest.app')

@section('content')
<section class="pt-120 pb-80">
    <div class="container-custom">

        {{-- ===================== --}}
        {{-- TITLE --}}
        {{-- ===================== --}}
        <div class="row">
            <div class="col-lg-12">
                <div class="section-title text-center mb-50">
                    <h2>Data Mutasi Aset</h2>
                    <p>Riwayat perubahan, pemindahan, atau penghapusan aset.</p>
                </div>
            </div>
        </div>

        {{-- SUCCESS MESSAGE --}}
        @if (session('success'))
            <div class="alert alert-success alert-dismissible fade show">
                {!! session('success') !!}
                <button type="button" class="btn-close" data-bs-dismiss="alert"></button>
            </div>
        @endif


        {{-- ===================== --}}
        {{-- STATISTIK MUTASI --}}
        {{-- ===================== --}}
        <div class="row mb-5">

            <div class="col-md-3">
                <div class="stats-card">
                    <div class="stats-number">{{ $mutasi->total() }}</div>
                    <div class="stats-label">Total Mutasi</div>
                </div>
            </div>

            <div class="col-md-3">
                <div class="stats-card" style="background: linear-gradient(135deg,#43e97b,#38f9d7);">
                    <div class="stats-number">
                        {{ $mutasi->where('jenis_mutasi','Pemindahan')->count() }}
                    </div>
                    <div class="stats-label">Pemindahan</div>
                </div>
            </div>

            <div class="col-md-3">
                <div class="stats-card" style="background: linear-gradient(135deg,#f093fb,#f5576c);">
                    <div class="stats-number">
                        {{ $mutasi->where('jenis_mutasi','Penghapusan')->count() }}
                    </div>
                    <div class="stats-label">Penghapusan</div>
                </div>
            </div>

            <div class="col-md-3">
                <div class="stats-card" style="background: linear-gradient(135deg,#4facfe,#00f2fe);">
                    <div class="stats-number">
                        {{ $mutasi->unique('aset_id')->count() }}
                    </div>
                    <div class="stats-label">Aset Termutasi</div>
                </div>
            </div>

        </div>


        {{-- ===================== --}}
        {{-- TOP BAR --}}
        {{-- ===================== --}}
        <div class="row mb-4 align-items-center">
            <div class="col-md-6">
                <a href="{{ route('mutasi.create') }}" class="main-btn btn-hover btn-sm">
                    <i class="fas fa-plus me-1"></i> Tambah Mutasi Aset
                </a>
            </div>

            <div class="col-md-6">
                <div class="search-box">
                    <form method="GET" action="{{ route('mutasi.index') }}">
                        <div class="row align-items-center">
                            <div class="col-md-6">
                                <h5 class="mb-0">Cari Mutasi</h5>
                            </div>

                            <div class="col-md-6">
                                <div class="input-group">
                                    <input type="text" name="search" class="form-control"
                                            placeholder="Cari nama aset, jenis mutasi..."
                                            value="{{ request('search') }}">
                                </div>
                            </div>
                        </div>
                    </form>
                </div>
            </div>
        </div>


        {{-- ===================== --}}
        {{-- LIST MUTASI --}}
        {{-- ===================== --}}
        <div class="row">
            @forelse ($mutasi as $m)
                <div class="col-xl-4 col-lg-6 col-md-6 mb-4">
                    <div class="card warga-card">

                        {{-- HEADER --}}
                        <div class="warga-header text-center">
                            <div class="warga-avatar mx-auto">
                                <i class="bi bi-arrow-left-right"></i>
                            </div>
                            <h5 class="mb-1">
                                {{ $m->aset->nama_aset ?? 'Aset Tidak Ditemukan' }}
                            </h5>
                            <p class="mb-0 opacity-75">
                                Kode: {{ $m->aset->kode_aset ?? '-' }}
                            </p>
                        </div>

                        {{-- DETAIL --}}
                        <div class="warga-info">

                            <div class="info-item">
                                <div class="info-icon"><i class="bi bi-calendar-event"></i></div>
                                <div class="info-content">
                                    <strong>Tanggal Mutasi</strong><br>
                                    {{ \Carbon\Carbon::parse($m->tanggal)->format('d M Y') }}
                                </div>
                            </div>

                            <div class="info-item">
                                <div class="info-icon"><i class="bi bi-shuffle"></i></div>
                                <div class="info-content">
                                    <strong>Jenis Mutasi</strong><br>
                                    <span class="badge bg-primary">{{ $m->jenis_mutasi }}</span>
                                </div>
                            </div>

                            <div class="info-item">
                                <div class="info-icon"><i class="bi bi-card-text"></i></div>
                                <div class="info-content">
                                    <strong>Keterangan</strong><br>
                                    {{ $m->keterangan ?: '-' }}
                                </div>
                            </div>

                        </div>

                        {{-- BUTTON ACTION --}}
                        <div class="action-buttons">
                            <div class="row">
                                <div class="col-6">
                                    <a href="{{ route('mutasi.edit', $m->mutasi_id) }}"
                                       class="btn btn-warning btn-sm text-white w-100">
                                        <i class="fas fa-edit me-1"></i>Edit
                                    </a>
                                </div>

                                <div class="col-6">
                                    <form action="{{ route('mutasi.destroy', $m->mutasi_id) }}"
                                          method="POST" class="d-inline w-100">
                                        @csrf
                                        @method('DELETE')
                                        <button class="btn btn-danger btn-sm w-100"
                                                onclick="return confirm('Hapus data mutasi ini?')">
                                            <i class="fas fa-trash me-1"></i>Hapus
                                        </button>
                                    </form>
                                </div>
                            </div>
                        </div>

                    </div>
                </div>

            @empty
                <div class="col-12">
                    <div class="card text-center py-5">
                        <i class="lni lni-map display-1 text-muted mb-3"></i>
                        <h4 class="text-muted">Belum ada data mutasi</h4>
                        <p class="text-muted">Tambahkan catatan mutasi aset pertama Anda.</p>
                        <a href="{{ route('mutasi.create') }}" class="main-btn btn-hover">
                            <i class="lni lni-plus"></i> Tambah Mutasi
                        </a>
                    </div>
                </div>
            @endforelse
        </div>

        {{-- PAGINATION --}}
        <div class="mt-3">
            {{ $mutasi->links('pagination::bootstrap-5') }}
        </div>

    </div>
</section>
@endsection
